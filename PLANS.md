# Plans

Active implementation work is tracked in [docs/active/](docs/active/).

## Clothing Item Resolution

- [x] Store concrete clothing definition names in new attachments
- [x] Match and restore clothing attachments through the clothing inventory API
- [x] Document compatibility and rollback limits for existing generic clothing attachments

## Construction Item Resolution

- [x] Store concrete construction definition names and texture variants in new attachments
- [x] Match and restore construction attachments through the construction inventory API
- [x] Preserve construction colors as part of durable attachment custody
- [x] Document compatibility and rollback limits for existing generic construction attachments
